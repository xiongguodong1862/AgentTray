...
BIN_PATH="$(swift build -c "$CONFIGURATION" --show-bin-path)/$BUILD_PRODUCT_NAME"
if [[ ! -x "$BIN_PATH" ]]; then
...
cp "$ROOT_DIR/AppBundle/CodexTray.icns" "$RESOURCES_DIR/CodexTray.icns"
